...
Identity: singleton identity |> andMap a = a
Homomorphism: singleton f |> andMap (singleton a) = singleton (f a)
Interchange: a |> andMap (singleton b) = singleton ((\b a -> a) b) |> andMap a
Composition: singleton (<<) |> andMap f |> andMap g |> andMap h = f |> andMap (g |> andMap h)

There's one more law, but it requires `map`, so we're skipping it:
Functor: map f x = singleton f |> andMap x

-}
andMapv1 : (number -> fa) -> (fab -> fa -> fa) -> (Fuzzer Float -> Fuzzer fa) -> Test
andMapv1 singleton andMap afuzz =
  let
    thing =
      number
  ...
  describe "andMapv1"
    [ fuzz thing "identity" <|
        \a -> (singleton identity <*> a) |> Expect.equal a
    , fuzz thing "homomorphism" <|
        \a -> (singleton f <*> singleton a) |> Expect.equal (singleton (f a))
    , fuzz2 thing thing "interchange" <|
        \a b -> (a <*> singleton b) |> Expect.equal (singleton ((\b a -> a) b) <*> a)
    , fuzz2 thing thing "composition" <|
        \a b -> (((singleton (<<) <*> f) <*> g) <*> h) |> Expect.equal (f <*> (g <*> h))
    ]
