...
comparables =
  describe "comparables"
    ...
    , fuzz (list comparable4) "comparable4" expectDuplicateFreeList
    , fuzz (list comparable5) "comparable5" expectDuplicateFreeList
    ]
  ...
  case lst of
    x :: xs ->
      if List.member x xs then
        Expect.fail ("found a duplicate element: `" ++ toString x ++ "` in `" ++ toString lst ++ "`")
